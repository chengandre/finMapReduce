"""
Main script for running FinQA QA using truncation baseline.

This script provides a command-line interface for running the truncation
baseline on FinQA data, allowing comparison with MapReduce results.
"""
...
def main():
    ...
    parser.add_argument('--doc_dir', type=str, default="../edgartools_finqa",
                      help='Directory containing FinQA markdown documents')
    ...
    print(f"  Doc directory: {args.doc_dir}")
    ...
    # Create FinQA truncation pipeline
    ...
    if args.verbose:
        ...
        for i, qa_item in enumerate(results["qa_data"]):
            print(f"\nQuestion {i+1}: {qa_item.get('question', 'N/A')}")
            print(f"Question type: {qa_item.get('question_type', 'N/A')}")
            print(f"Document: {pipeline.get_document_identifier(qa_item)}")
