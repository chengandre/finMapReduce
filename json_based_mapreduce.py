...
class JSONBasedMapReduce(BaseMapReduceQA):
    """
    Intermediate abstract class for pipelines using JSON-structured prompts and responses.

    This class provides common implementation for pipelines that:
    - Use the GPT wrapper with JSON parsing
    - Filter results based on relevance scores
    - Format results as XML for the reduce phase
    ...
    def invoke_llm_map(self, chunk: Any, question: str) -> Dict[str, Any]:
        ...
        return self.llm(map_prompt, context=chunk.page_content, final_query=question)
    ...
    def invoke_llm_reduce(self, formatted_results: Any, question: str) -> Any:
        ...
        return self.llm(reduce_prompt, map_results=formatted_results, final_query=question)

    def preprocess_map_results(self, results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Filter map results based on relevance score.

        Only keeps results with relevance_score > 6.
    ...
    def format_map_results_for_reduce(self, results: List[Any], question: str) -> str:
        """
    ...
    def parse_final_result(self, result: Any) -> Dict[str, Any]:
        """
        Parse the final result from reduce phase.
